...
- **银幕反打冷光**（顶部洒下的放映光，暗色主题偏青、明亮主题偏朱砂）+ **四角压暗** + **底边前排观众剪影**（极淡，只留一线轮廓）。
- **空状态大幅底图**：新会话界面铺开 AI 素材，每 24 秒交叉淡入换一张，8 张全员轮播。底图带**中心留白遮罩**——正文与输入区所在的中轴永不着色，可读性由构造保证而不是靠压透明度。
- **门厅海报牌**：新会话界面左下角的小立牌，循环展示当前"上映中"的素材并报出片名（治愈系水墨国风 / 满座的抽象同类 / 良币的沉默 …）。进入对话后自动收起。
- **水印**：侧栏一枚极淡朱砂「牛来」印，右侧边一只低模牛在看着你。
- 窄视口（≤1100px）自动隐藏立牌与侧栏印；`niulai.decor(false)` 或 `niulai.art="calm"` 可关闭/减弱；`niulai.next()` 立刻换下一张。
