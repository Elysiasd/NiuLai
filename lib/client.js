  ...
  factory: (require) => {
    ...
    const THEME_CSS = `
      body:not([data-ds-dark-theme]) {
        ...
        --dsw-alias-interactive-bg-active: rgba(255, 160, 77, 0.18) !important;
        --dsw-alias-interactive-bg-hover-accent: rgba(255, 153, 102, 0.16) !important;
        --dsw-alias-interactive-bg-hover-danger: rgba(220, 50, 47, 0.08) !important;
        --dsw-alias-button-primary-fill: #ff9966 !important;
        --dsw-alias-button-primary-hover: #ffa04d !important;
      ...
      body[data-ds-dark-theme] {
        ...
        --dsw-alias-interactive-bg-active: rgba(255, 160, 77, 0.22) !important;
        --dsw-alias-button-primary-fill: #ff9966 !important;
        --dsw-alias-button-primary-hover: #ffa04d !important;
      ...
      /* dsh 品牌位接管（restyleBrandMarks 注入的节点）：
         侧栏 logo 换「牛来影厅 · 手搓版」lockup，hero 标题行换牛头 + 楷体。 */
      .niulai-brand {
        display: inline-flex; align-items: center; gap: 7px;
        font-family: "Kaiti SC", "STKaiti", KaiTi, serif;
      }
      .niulai-brand-seal { width: 20px; height: 20px; flex: none; }
      .niulai-brand-name {
        font-size: 17px; font-weight: 700; letter-spacing: 0.08em; color: #2f2a24;
      }
      body[data-ds-dark-theme] .niulai-brand-name { color: #f7f2e7; }
      .niulai-brand-badge {
        font-size: 10px; letter-spacing: 0.14em; padding: 1px 6px; border-radius: 4px;
        color: #b3392f; box-shadow: inset 0 0 0 1px currentColor; flex: none;
      }
      .niulai-hero-cow { width: 30px; height: 30px; vertical-align: -6px; }
      [class*="_headlineText"] {
    ...
    // ══ 常驻布景（niulai.decor）：让影厅在不干活时也在场 ══
    // 全层 pointer-events:none + 低透明度 + 混合模式，只当氛围不挡操作。
    const DECOR_CSS = `
      #niulai-decor { position: fixed; inset: 0; pointer-events: none; z-index: 2147480500; }
      #niulai-decor > * { position: absolute; }
      /* 银幕反打冷光：从顶部洒下来的放映光 */
      #niulai-decor .nd-glow {
        top: 0; left: 0; right: 0; height: 46vh;
        background: radial-gradient(120% 100% at 50% -20%, rgba(125, 214, 198, 0.20), transparent 62%);
      }
      body:not([data-ds-dark-theme]) #niulai-decor .nd-glow {
        background: radial-gradient(120% 100% at 50% -18%, rgba(179, 57, 47, 0.10), transparent 60%);
      }
      /* 影厅四角压暗 */
      #niulai-decor .nd-vignette {
        inset: 0;
        background: radial-gradient(130% 105% at 50% 45%, transparent 55%, rgba(11, 11, 11, 0.34) 100%);
      }
      body:not([data-ds-dark-theme]) #niulai-decor .nd-vignette {
        background: radial-gradient(130% 105% at 50% 45%, transparent 56%, rgba(96, 86, 60, 0.22) 100%);
      }
      /* 水墨远山：海报承诺的那个国风，压在界面底边当地平线 */
      #niulai-decor .nd-ridge {
        left: 0; right: 0; bottom: 0; height: 132px; opacity: 0.5;
        background-repeat: no-repeat; background-size: 100% 100%;
        background-position: bottom center;
      }
      body[data-ds-dark-theme] #niulai-decor .nd-ridge { opacity: 0.42; }
      /* 底边前排观众：坐在山脊线上的一排牛头，只露头肩，不裁半张脸。
         背景整幅贴底（不再负偏移），高度取剪影原始比例，避免裁出碎块。 */
      #niulai-decor .nd-audience {
        left: 0; right: 0; bottom: 0; height: 96px; opacity: 0.5;
        background-repeat: repeat-x; background-size: auto 100%;
        background-position: bottom left 24px;
      }
      body[data-ds-dark-theme] #niulai-decor .nd-audience { opacity: 0.62; }
      /* 闲时弹幕：影厅里此起彼伏的窃私语，间隔很长，只走上部 */
      #niulai-decor .nd-murmur {
        white-space: nowrap; left: 100vw;
        font-family: "Kaiti SC", "STKaiti", KaiTi, sans-serif;
        font-size: 19px; font-weight: 700; color: #b3392f; opacity: 0.5;
        animation: niulai-murmur-fly linear forwards;
      }
      body[data-ds-dark-theme] #niulai-decor .nd-murmur {
        color: #f7f2e7; opacity: 0.42; text-shadow: 0 0 6px rgba(0,0,0,.6);
      }
      @keyframes niulai-murmur-fly { to { transform: translateX(calc(-100vw - 100%)); } }
      /* 空状态大幅轮播底图：双层交叉淡入，柔焦到只剩气氛 */
      #niulai-decor .nd-backdrop {
        inset: 0; background-repeat: no-repeat; background-position: center 38%;
        background-size: cover; opacity: 0; transition: opacity 2.2s ease;
        filter: blur(2px) saturate(0.85);
        mix-blend-mode: multiply;
        /* 中心留白：正文与输入区所在的中轴不着色，只有四周显影。
           这样底图永远不压文字对比度，可读性由构造保证而非靠调透明度。 */
        -webkit-mask-image: radial-gradient(115% 88% at 50% 46%, transparent 42%, #000 78%);
        mask-image: radial-gradient(115% 88% at 50% 46%, transparent 42%, #000 78%);
      }
      body[data-ds-dark-theme] #niulai-decor .nd-backdrop {
        mix-blend-mode: screen; filter: blur(2px) saturate(0.8) brightness(0.7);
      }
      #niulai-decor.nd-hero .nd-backdrop.nd-on { opacity: 0.13; }
      body[data-ds-dark-theme] #niulai-decor.nd-hero .nd-backdrop.nd-on { opacity: 0.20; }
      /* 门厅海报立牌：循环展示 AI 素材并报出片名。
         只在对话消息流视图出现（nd-chat 白名单）——此时 composer 是底部固定条、
         左侧留白是真空的。空状态另有全屏轮播底图担任展示位，不出立牌，
         避免压住居中且很宽的 hero 控件（牛来模式选择器就在那条带上）；
         轨迹页的表格横贯全宽、同样容不下立牌——探测不到消息流就不出，宁缺毋误报。 */
      #niulai-decor .nd-lobby { display: none; }
      #niulai-decor.nd-chat .nd-lobby { display: block; }
      #niulai-decor .nd-lobby {
        left: 280px; bottom: 26px; width: 196px;
        transform: rotate(-1.4deg); opacity: 0; transition: opacity 1.1s ease;
        border-radius: 6px; overflow: hidden; background: #f7f2e7;
        box-shadow: 0 10px 30px rgba(0, 0, 0, 0.22), 0 0 0 1px rgba(96, 86, 60, 0.3);
      }
      #niulai-decor .nd-lobby.nd-on { opacity: 1; }
      body[data-ds-dark-theme] #niulai-decor .nd-lobby { background: #1b2823; }
      body[data-ds-dark-theme] #niulai-decor .nd-lobby.nd-on { opacity: 0.94; }
      #niulai-decor .nd-lobby-img {
        position: relative; display: block; width: 100%; height: 114px;
        background-repeat: no-repeat; background-position: center; background-size: cover;
        transition: opacity 1.6s ease;
      }
      #niulai-decor .nd-lobby-cap {
        position: relative; display: flex; align-items: center; gap: 6px;
        padding: 5px 8px 6px;
        font-family: "Kaiti SC", "STKaiti", KaiTi, serif; font-size: 12px; line-height: 1.35;
        color: #2f2a24; border-top: 1px solid rgba(96, 86, 60, 0.22);
      }
      body[data-ds-dark-theme] #niulai-decor .nd-lobby-cap {
        color: #e7dcc0; border-top-color: rgba(231, 220, 192, 0.18);
      }
      #niulai-decor .nd-lobby-seal { width: 15px; height: 15px; flex: none; }
      #niulai-decor .nd-lobby-title { flex: 1; min-width: 0; }
      #niulai-decor .nd-lobby-now {
        font-size: 10px; letter-spacing: .12em; color: #b3392f; flex: none;
      }
      /* 侧栏与输入区水印：朱砂印 + 低模牛，常驻不动 */
      /* 侧栏朱砂印：盖在侧栏中下部的空白上，像盖在宣纸上的藏印 */
      #niulai-decor .nd-sidebar-seal {
        left: 74px; top: 46%; width: 96px; height: 96px; opacity: 0.2;
        transform: rotate(-6deg);
      }
      body[data-ds-dark-theme] #niulai-decor .nd-sidebar-seal { opacity: 0.26; }
      /* 低模牛：从右侧边探头进来看着你（初生牛犊在场）。
         贴边偏上，避开输入区的发送按钮与模型选择器。 */
      #niulai-decor .nd-cow {
        right: -22px; top: 34%; width: 148px; height: 148px; opacity: 0.5;
        transform: rotate(8deg);
      }
      body[data-ds-dark-theme] #niulai-decor .nd-cow { opacity: 0.4; }
      @media (max-width: 1300px) { #niulai-decor .nd-cow { display: none; } }
      html[data-niulai-crude] #niulai-decor .nd-backdrop,
      html[data-niulai-crude] #niulai-decor .nd-lobby-img {
        filter: saturate(1.9) contrast(1.2) hue-rotate(8deg); image-rendering: pixelated;
      }
      /* 视口不够宽时正文两侧没有留白：立牌与侧栏印一律不出，避免压内容。
         选择器与上面的 .nd-chat display:block 同特异性，靠源顺序压住它。 */
      @media (max-width: 1400px) { #niulai-decor.nd-chat .nd-lobby { display: none; } }
      @media (max-width: 1100px) { #niulai-decor .nd-sidebar-seal { display: none; } }
      /* 电影字幕条：底部黑底白字标注，模拟《牛来》电影截图里的字幕样式 */
      #niulai-decor .nd-subtitle {
        left: 0; right: 0; bottom: 0; height: 32px;
        background: rgba(0, 0, 0, 0.85); color: #fff;
        font-family: "PingFang SC", "Microsoft YaHei", sans-serif;
        font-size: 13px; line-height: 32px; padding: 0 20px;
        display: flex; align-items: center; gap: 8px; opacity: 0.88;
        z-index: 2147483600; pointer-events: none;
      }
      #niulai-decor .nd-subtitle span {
        font-weight: 500; letter-spacing: 0.05em;
      }
      @media (prefers-reduced-motion: reduce) {
        #niulai-decor .nd-backdrop, #niulai-decor .nd-lobby, #niulai-decor .nd-lobby-img { transition: none !important; }
      }
    `;
    ...
    function apply(ctx) {
      ...
      const decorOff = safeGet(DECOR_KEY) === "off";
      ...
      const artInit = () => {
        ...
        artStyleEl.textContent = ART_CSS + DECOR_CSS;
      ...
      // ── 常驻布景：氛围层 + 门厅海报牌 + 空状态轮播底图 ──
      let decorEl = null;
      let decorTimer = null;
      let decorHeroWatch = null;
      let decorIndex = 0;
      let murmurTimer = null;
      /** 交叉淡入用的双层底图，指向当前显示的那层。 */
      let backdropTop = false;

      /** 空状态（新会话）判定：hero 槽位只在没有消息流时存在。 */
      const isHero = () =>
        document.querySelector('[data-slot="conversation.hero.workspace"]') !== null;
      /** 对话消息流判定：立牌显示的白名单条件。轨迹页没有 [data-chat-flow]，
          设置等未知视图同理——探测不到就不出牌，宁缺毋误报。 */
      const isChatFlow = () => {
        const flow = document.querySelector("[data-chat-flow]");
        return flow !== null && flow.getBoundingClientRect().width > 0;
      };

      const buildDecor = () => {
        if (decorEl || artLevel === "off") return;
        const rotation = manifest?.decor?.rotation;
        if (!rotation || rotation.length === 0) return;

        decorEl = document.createElement("div");
        decorEl.id = "niulai-decor";
        decorEl.setAttribute("aria-hidden", "true");
        const seal = assetUrl(manifest?.seals?.niulai || "seals/niulai.svg");
        decorEl.innerHTML =
          '<div class="nd-backdrop" data-layer="a"></div>' +
          '<div class="nd-backdrop" data-layer="b"></div>' +
          '<div class="nd-glow"></div>' +
          '<div class="nd-vignette"></div>' +
          `<div class="nd-ridge" style="background-image:url('${assetUrl("sprites/ridge.svg")}')"></div>` +
          `<div class="nd-audience" style="background-image:url('${assetUrl(manifest?.audienceStrip || "sprites/audience-strip.svg")}')"></div>` +
          '<div class="nd-subtitle"><span>国产电影《牛来》</span></div>' +
          `<img class="nd-sidebar-seal" alt="" src="${seal}">` +
          `<img class="nd-cow" alt="" src="${assetUrl("sprites/cow-head.svg")}">` +
          '<div class="nd-lobby">' +
          '<span class="nd-lobby-img"></span>' +
          '<div class="nd-lobby-cap">' +
          `<img class="nd-lobby-seal" alt="" src="${seal}">` +
          '<span class="nd-lobby-title"></span>' +
          '<span class="nd-lobby-now">上映中</span>' +
          "</div></div>";
        document.body.appendChild(decorEl);

        const layers = decorEl.querySelectorAll(".nd-backdrop");
        const lobby = decorEl.querySelector(".nd-lobby");
        const lobbyImg = decorEl.querySelector(".nd-lobby-img");
        const lobbyTitle = decorEl.querySelector(".nd-lobby-title");

        const show = (index) => {
          const item = rotation[index % rotation.length];
          const url = assetUrl(item.img);
          // 空状态底图：两层交叉淡入，避免切换时闪一下背景色。
          const incoming = layers[backdropTop ? 0 : 1];
          const outgoing = layers[backdropTop ? 1 : 0];
          incoming.style.backgroundImage = `url("${url}")`;
          incoming.classList.add("nd-on");
          outgoing.classList.remove("nd-on");
          backdropTop = !backdropTop;
          lobbyImg.style.backgroundImage = `url("${url}")`;
          lobbyTitle.textContent = item.title;
          lobby.classList.add("nd-on");
        };

        show(decorIndex);
        // calm 档只挂一张，不轮播（reduced-motion 同理）。
        if (artLevel === "full" && !reducedMotion) {
          const period = manifest.decor.rotationMs || 24000;
        ...
        // 视图轮询：空状态铺满屏底图（nd-hero），对话消息流出立牌（nd-chat）。
        const syncHero = () => {
          if (!decorEl) return;
          const hero = isHero();
          decorEl.classList.toggle("nd-hero", hero);
          decorEl.classList.toggle("nd-chat", !hero && isChatFlow());
        };
        syncHero();
        decorHeroWatch = setInterval(syncHero, 1200);

        // 闲时窃私语：影厅里零星飘过的笑声，间隔 50–95 秒随机，一次一条。
        // 与完成时刻的弹幕爆发是两套预算，互不占用；回合进行中不打扰。
        if (artLevel === "full" && !reducedMotion && danmakuOn && danmaku) {
          const murmur = () => {
            if (!decorEl || adopted.size > 0) return;
            const span = document.createElement("span");
            span.className = "nd-murmur";
            span.textContent = weightedPick(danmaku.words);
            span.style.top = `${14 + Math.random() * 26}vh`;
            span.style.animationDuration = `${9000 + Math.random() * 4000}ms`;
            decorEl.appendChild(span);
            setTimeout(() => span.remove(), 14000);
          };
          murmurTimer = setInterval(() => {
            if (Math.random() < 0.55) murmur();
      ...
      const publicApi = {
        ...
        decor(on) {
          if (on === false) safeSet(DECOR_KEY, "off");
          else {
            try {
              localStorage.removeItem(DECOR_KEY);
            } catch (error) {
              /* ignore */
            }
          }
        ...
        /** 立刻换下一张门厅海报（不等 24 秒轮播）。 */
        next() {
          if (!decorEl) return "布景层未启用";
          decorIndex++;
          const rotation = manifest?.decor?.rotation || [];
          const item = rotation[decorIndex % rotation.length];
          const url = assetUrl(item.img);
          const layers = decorEl.querySelectorAll(".nd-backdrop");
          const incoming = layers[backdropTop ? 0 : 1];
          const outgoing = layers[backdropTop ? 1 : 0];
          ...
          outgoing.classList.remove("nd-on");
          backdropTop = !backdropTop;
          decorEl.querySelector(".nd-lobby-img").style.backgroundImage = `url("${url}")`;
          decorEl.querySelector(".nd-lobby-title").textContent = item.title;
          return item.title;
        },
      };

      // ── 品牌替换：dsh 原生 favicon 与标题换成《牛来》 ──
      // favicon 换成「牛来」朱文方印（seals/niulai.svg，楷体艺术字印章），
      // 标题里的产品名换成「牛来影厅」。dsh 切换会话时会重写 document.title，
      // 用 MutationObserver 持续接管；替换后的标题不再含原词，不会自触发循环。
      // 跟随主题开关（niulai.theme=off 时不接管），卸载时按记录还原。
      const BRAND_SOURCE = "DeepSeek Harness";
      const BRAND_NAME = "牛来影厅";
      let titleWatch = null;
      const faviconRestore = [];
      const applyBrand = () => {
        const url = ASSET_BASE + "seals/niulai.svg";
        let links = Array.from(document.querySelectorAll('link[rel~="icon"]'));
        if (links.length === 0) {
          const link = document.createElement("link");
          link.rel = "icon";
          document.head.appendChild(link);
          faviconRestore.push({ link, created: true, href: null, type: null });
          links = [link];
        } else {
        ...
        const retitle = () => {
          if (document.title.includes(BRAND_SOURCE)) {
            document.title = document.title.replaceAll(BRAND_SOURCE, BRAND_NAME);
          }
        };
        retitle();
        const titleEl = document.querySelector("title");
        if (titleEl !== null) {
          titleWatch = new MutationObserver(retitle);
          titleWatch.observe(titleEl, { childList: true, characterData: true, subtree: true });
        }
      };

      // dsh 自有品牌位（侧栏 logo、hero 标题行）的接管。这些节点随视图挂载
      // 反复重建，用 1.5s 幂等重扫补妆；类名是构建期 hash，只按语义后缀
      // （_brand/_fishHitbox/_headlineText/_previewBadge）匹配，
      // 上游改版找不到就什么都不做，失效方向是保留原生外观、零误伤。
      // 原文与原节点都留档（dataset / display），卸载时按档还原。
      let brandRescan = null;
      const restyleBrandMarks = () => {
        // 侧栏 logo：deepseek harness 字标 → 「牛来影厅 · 手搓版」lockup
        for (const brand of document.querySelectorAll('[class*="_brand"]')) {
          if (brand.querySelector(".niulai-brand") !== null) continue;
          const logo = brand.querySelector("svg");
          if (logo === null) continue;
          logo.style.display = "none";
          const mark = document.createElement("span");
          mark.className = "niulai-brand";
          const seal = document.createElement("img");
          seal.className = "niulai-brand-seal";
          seal.alt = "";
          seal.src = ASSET_BASE + "seals/niulai.svg";
          const markName = document.createElement("span");
          markName.className = "niulai-brand-name";
          markName.textContent = BRAND_NAME;
          const markBadge = document.createElement("span");
          markBadge.className = "niulai-brand-badge";
          markBadge.textContent = "手搓版";
          mark.append(seal, markName, markBadge);
          brand.appendChild(mark);
        }
        // hero 标题行：鲸鱼 → 低模牛头，「探索未至之境」→ 片名「牛来」，
        // 「预览版」→「上映中」（与门厅立牌同一句影厅语）。
        const fishBox = document.querySelector('[class*="_fishHitbox"]');
        if (fishBox !== null && fishBox.querySelector(".niulai-hero-cow") === null) {
          const fish = fishBox.querySelector("svg");
          if (fish !== null) fish.style.display = "none";
          const cow = document.createElement("img");
          cow.className = "niulai-hero-cow";
          cow.alt = "";
          cow.src = ASSET_BASE + "sprites/cow-head.svg";
          fishBox.appendChild(cow);
        }
        const headline = document.querySelector('[class*="_headlineText"]');
        if (headline !== null) {
          if (headline.dataset.niulaiOrig === undefined) {
            headline.dataset.niulaiOrig = headline.textContent;
          }
          if (headline.textContent !== "牛来") headline.textContent = "牛来";
        }
        const previewBadge = document.querySelector('[class*="_previewBadge"]');
        if (previewBadge !== null) {
          if (previewBadge.dataset.niulaiOrig === undefined) {
            previewBadge.dataset.niulaiOrig = previewBadge.textContent;
          }
          if (previewBadge.textContent !== "上映中") previewBadge.textContent = "上映中";
        }
      };
      ...
      const start = () => {
        if (!themeOff) {
          ...
          applyBrand();
          restyleBrandMarks();
          brandRescan = setInterval(restyleBrandMarks, 1500);
        }
      ...
      ctx.effect(() => {
        return () => {
          ...
          if (decorTimer !== null) clearInterval(decorTimer);
          if (murmurTimer !== null) clearInterval(murmurTimer);
          if (decorHeroWatch !== null) clearInterval(decorHeroWatch);
          if (decorEl !== null && decorEl.isConnected) decorEl.remove();
          decorEl = null;
          ...
          if (brandRescan !== null) {
            clearInterval(brandRescan);
            brandRescan = null;
          }
          for (const mark of document.querySelectorAll(".niulai-brand, .niulai-hero-cow")) {
            mark.remove();
          }
          for (const hidden of document.querySelectorAll(
            '[class*="_brand"] svg, [class*="_fishHitbox"] svg'
          )) {
            hidden.style.removeProperty("display");
          }
          for (const sel of ['[class*="_headlineText"]', '[class*="_previewBadge"]']) {
            const el = document.querySelector(sel);
            if (el !== null && el.dataset.niulaiOrig !== undefined) {
              el.textContent = el.dataset.niulaiOrig;
              delete el.dataset.niulaiOrig;
            }
          }
          if (document.title.includes(BRAND_NAME)) {
            document.title = document.title.replaceAll(BRAND_NAME, BRAND_SOURCE);
          }
          for (const f of faviconRestore) {
            if (f.created) f.link.remove();
            else {
              if (f.href !== null) f.link.setAttribute("href", f.href);
              else f.link.removeAttribute("href");
              if (f.type !== null) f.link.setAttribute("type", f.type);
              else f.link.removeAttribute("type");
            }
          }
          faviconRestore.length = 0;
