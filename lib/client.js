  ...
  factory: (require) => {
    ...
    const DECOR_CSS = `
      ...
      #niulai-decor .nd-glow {
        ...
        background: radial-gradient(120% 100% at 50% -20%, rgba(125, 214, 198, 0.16), transparent 62%);
      }
      body:not([data-ds-dark-theme]) #niulai-decor .nd-glow {
        background: radial-gradient(120% 100% at 50% -20%, rgba(179, 57, 47, 0.07), transparent 62%);
      }
      /* 影厅四角压暗 */
      #niulai-decor .nd-vignette {
        inset: 0;
        background: radial-gradient(130% 105% at 50% 45%, transparent 55%, rgba(11, 11, 11, 0.30) 100%);
      }
      body:not([data-ds-dark-theme]) #niulai-decor .nd-vignette {
        background: radial-gradient(130% 105% at 50% 45%, transparent 58%, rgba(96, 86, 60, 0.16) 100%);
      }
      /* 底边前排观众：常驻但极淡，只留一线轮廓 */
      #niulai-decor .nd-audience {
        left: 0; right: 0; bottom: 0; height: 82px; opacity: 0.16;
        background-repeat: repeat-x; background-size: auto 100%;
        background-position: bottom -34px left 40px;
      }
      body[data-ds-dark-theme] #niulai-decor .nd-audience { opacity: 0.30; }
      ...
      /* 门厅海报牌：循环展示 AI 素材并报出片名。
         只在空状态（新会话）出现，落在会话区左下象限——侧栏之右、输入区之下，
         那里在新会话界面是真空着的。进入对话后自动收起，避免长期压住任何内容。 */
      #niulai-decor .nd-lobby { display: none; }
      #niulai-decor.nd-hero .nd-lobby { display: block; }
      #niulai-decor .nd-lobby {
        left: 34vw; bottom: 13vh; width: 178px;
        transform: rotate(-1.4deg); opacity: 0; transition: opacity 1.1s ease;
        border-radius: 5px; overflow: hidden; background: #f7f2e7;
        box-shadow: 0 8px 26px rgba(0, 0, 0, 0.28), 0 0 0 1px rgba(96, 86, 60, 0.28);
      }
      #niulai-decor .nd-lobby.nd-on { opacity: 0.9; }
      body[data-ds-dark-theme] #niulai-decor .nd-lobby { background: #1b2823; }
      body[data-ds-dark-theme] #niulai-decor .nd-lobby.nd-on { opacity: 0.82; }
      ...
      #niulai-decor .nd-sidebar-seal {
        left: 96px; bottom: 108px; width: 54px; height: 54px; opacity: 0.09;
      }
      body[data-ds-dark-theme] #niulai-decor .nd-sidebar-seal { opacity: 0.16; }
      /* 低模牛：贴右侧边中部，避开输入区的发送按钮与模型选择器 */
      #niulai-decor .nd-cow {
        right: 14px; top: 50%; margin-top: -17px; width: 34px; height: 34px; opacity: 0.3;
      }
      ...
      /* 窄视口没有富余空间：立牌与侧栏印一律不出。 */
      @media (max-width: 1100px) {
        #niulai-decor.nd-hero .nd-lobby, #niulai-decor .nd-sidebar-seal { display: none; }
      }
      @media (prefers-reduced-motion: reduce) {
    ...
    function apply(ctx) {
      ...
      const buildDecor = () => {
        ...
        // 空状态出现/消失时切换 hero 类：只有空状态才铺满屏底图。
        const syncHero = () => {
          if (!decorEl) return;
          decorEl.classList.toggle("nd-hero", isHero());
        };
        syncHero();
